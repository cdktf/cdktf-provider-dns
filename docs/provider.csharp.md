...
```csharp
public DnsProviderUpdate UpdateInput { get; }
```
...
```csharp
public DnsProviderUpdate Update { get; }
```
...
```csharp
public DnsProviderUpdate Update { get; set; }
```
...
new DnsProviderUpdate {
    string Server,
    DnsProviderUpdateGssapi Gssapi = null,
...
| <code><a href="#@cdktf/provider-dns.provider.DnsProviderUpdate.property.port">Port</a></code> | <code>double</code> | The target UDP port on the server where updates are sent to. Defaults to `53`. |
| <code><a href="#@cdktf/provider-dns.provider.DnsProviderUpdate.property.retries">Retries</a></code> | <code>double</code> | How many times to retry on connection timeout. Defaults to `3`. |
...
```csharp
public DnsProviderUpdateGssapi Gssapi { get; set; }
```
